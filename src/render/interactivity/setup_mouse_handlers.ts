...
export function setupMouseHandlers(
    { canvasRef, projectStore, mouseState }:
    {
    ...
    const getLayerAtPosition = (screenX: number, screenY: number): Layer | null => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
        if (!mouseState.isDragging || !mouseState.dragStart || !projectStore.selectedLayer) return;
        ...
        projectStore.updateLayerPosition(projectStore.selectedLayer.id, newPosition);
